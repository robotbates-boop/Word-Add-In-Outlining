...
(function () {
  ...
  // Register immediately (before any Word/Office calls)
  window.WordToolkit.register("dynamicNumberingToText", async ({ setStatus }) => {
    ...
    await Word.run(async (context) => {
      ...
      // B) Convert fields to plain text (skip if API not available)
      let fieldCount = 0;
      ...
      try {
        ...
        const fields = body.fields; // <-- this can throw ApiNotFound on some builds
        fields.load("items");
        await context.sync();
        ...
        // This is the important part: do NOT fail the whole module.
